...
# 7) Harden PIF autopif.sh: eliminate ALL possible MODPATH writes
#    a) TEMPDIR fallback -> CONFIG_DIR
#    b) tee -> absolute $TEMPDIR path (immune to cd failure)
#    c) cd guard: exit if cd fails (prevent silent write to CWD=MODPATH)
