...
# Each tracked PID is written to $PID_FILE for external monitoring/debugging.
PID_FILE="$MODDIR/.tiejia_bg_pids"
: > "$PID_FILE" 2>/dev/null  # clear stale file on each boot
...
# Backward compat: CONFIG_DIR defined by common_func.sh as TIEJIA_CONFIG_DIR
# but early code may use CONFIG_DIR directly. Ensure both are set.
[ -z "${CONFIG_DIR:-}" ] && CONFIG_DIR="${TIEJIA_CONFIG_DIR:-/data/adb/tricky_store}"
...
# Prevents detection apps from reading dmesg for Magisk/SuperSU boot traces.
# Setting persists until next reboot; service.sh re-applies each boot.
...
# --- Developer options state hiding (settings provider) ---
# Many detection apps check Settings.Global.DEVELOPMENT_SETTINGS_ENABLED
# to infer a tampered/developer device profile.
# settings put requires the settings provider to be ready (post-boot).
settings put global development_settings_enabled 0 2>/dev/null || true
# NOTE: adb_enabled is deliberately NOT written here — it persists across
# reboots and would prevent the user from re-enabling ADB. Runtime hiding
# is already handled above via setprop ctl.stop adbd + resetprop.

# --- ROM fingerprint deep scrub (Specter-style keyword scan) ---
...
# --- Boot hash (zero-rejection priority chain) ---
# Run early so TEE simulator has device identity hash before keybox init.
...
# kick running banking apps for cosmetic refreshes.
{
    ...
    while true; do
        ...
        [ "$INT" -lt 60 ] && INT=60
        sleep "$INT"
        # Hourly refresh — run fingerprint and keybox pipelines in parallel
        # (they are independent: pif writes pif.prop, keybox writes keybox.xml)
        ...
        wait  # finish both pipelines before status
        # Target cleanup — removes uninstalled apps from target.txt every 6h
        if [ -x "$MODDIR/target_cleanup.sh" ]; then
            LAST_CLEAN=$(cat "$CFG/.last_target_cleanup" 2>/dev/null)
            NOW=$(date +%s)
            if [ -z "$LAST_CLEAN" ] || [ $((NOW - LAST_CLEAN)) -gt 21600 ]; then
                MODPATH="$MODDIR" sh "$MODDIR/target_cleanup.sh" >/dev/null 2>&1
                echo "$NOW" > "$CFG/.last_target_cleanup" 2>/dev/null
            fi
        fi
