...
# Last resort: use current month if device prop is also empty/broken
# (e.g., fresh ROM flash or custom ROM without the prop). This prevents
# TEE attestation from using an arbitrarily old or zero patch level.
if [ -z "$SP" ]; then
    SP=$(date '+%Y-%m')-05 2>/dev/null
    [ -z "$SP" ] && SP="2024-01-05"
fi
