...
EOF
  ...
  # toggle.val override: WebUI-written file (id=action per line) takes
  # precedence over the static conflicts.txt type. This is the single
  # source of truth for conflict handling decisions — a user can demote
  # an "aggressive" entry to "passive" or disable it entirely.
  # Format: module_id=keep|aggressive|moderate|passive
  tg_file="${CFG}/config/toggle.val"
  if [ -f "$tg_file" ]; then
    tg_line=$(grep "^${id}=" "$tg_file" 2>/dev/null | tail -1)
    if [ -n "$tg_line" ]; then
      tg_val="${tg_line#*=}"
      case "$tg_val" in
        keep)      continue ;;           # user exempts this module entirely
        aggressive|moderate|passive) type="$tg_val" ;;
        *)         ;;                     # unknown value, keep conflicts.txt type
      esac
    fi
  fi
